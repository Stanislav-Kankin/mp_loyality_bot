...
async def get_shop_welcome(pool: asyncpg.Pool, shop_id: int) -> dict | None:
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            """
            SELECT id, welcome_text, welcome_photo_file_id
            FROM shops
            WHERE id=$1;
            """,
            shop_id,
        ...
        return {
            "id": int(row["id"]),
            "welcome_text": str(row["welcome_text"]) if row["welcome_text"] is not None else "",
            "welcome_photo_file_id": str(row["welcome_photo_file_id"]) if row["welcome_photo_file_id"] is not None else "",
        }
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute(
            ...
            SET
                ...
                welcome_photo_file_id = COALESCE($4, welcome_photo_file_id)
            WHERE id=$2;
            """,
            seller_tg_user_id,
            shop_id,
...
async def get_shop_for_seller(pool: asyncpg.Pool, seller_tg_user_id: int, shop_id: int) -> dict | None:
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            """
            SELECT sh.id, sh.name, sh.category, sh.is_active, sh.created_at, sh.welcome_text, sh.welcome_photo_file_id
            FROM shops sh
            JOIN sellers s ON s.id = sh.seller_id
            WHERE s.tg_user_id=$1 AND sh.id=$2;
            """,
...
async def list_seller_campaigns(pool: asyncpg.Pool, *, seller_tg_user_id: int, limit: int = 10) -> list[dict]:
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            """
            SELECT c.id, c.status, c.created_at, sh.name AS shop_name
            FROM campaigns c
        ...
        return [{"id": int(r["id"]), "status": str(r["status"]), "created_at": r["created_at"], "shop_name": str(r["shop_name"])} for r in rows]
...
async def get_campaign_for_seller(
    ...
    async with pool.acquire() as conn:
        r = await conn.fetchrow(
            """
            SELECT c.id, c.status, c.created_at, sh.name AS shop_name, c.text, c.button_title, c.url, c.price_minor, c.currency
            FROM campaigns c
            JOIN shops sh ON sh.id = c.shop_id
            JOIN sellers s ON s.id = sh.seller_id
        ...
        return {
            ...
            "photo_file_id": str(r["photo_file_id"]) if r["photo_file_id"] is not None else "",
